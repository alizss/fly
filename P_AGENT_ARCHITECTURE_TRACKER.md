...
Current phase: `P0.9 canonical logical control graph`
...
| P0.4 | Add transaction invariants. | `[x]` | Pre-payment invariants block payment/final actions, unsafe money actions, validation-error navigation, unresolved required sections, stale active-surface/background navigation, and observation-hash drift. If navigation is proposed while a no-extras paid-extra decision is still pending, the loop now recovers to the visible safe decline choice before policy/execution. Invariants consume typed intent/target snapshots: `decline_optional_extra` and `open_choice_control` are allowed, paid selection remains blocked. | Cancellation guarantee test showed the planner could say a paid-extra choice was pending while still proposing `Continue`; the invariant blocked it correctly, but the loop needed recovery to the visible `No thanks` control. | Payment authorization remains separate in P0.6 before any real payment/final purchase action. |
...
| P0.8 | Evidence-backed requirement reconciliation. | `[x]` | Blind `satisfiedRequirementIds` merging is removed. Authority order is now deterministic current control/section/task state -> evidence-backed current-observation verifier update -> classifier/verifier interpretation -> historical session memory. Contradictions without deterministic evidence become `conflicted`; backend policy blocks Continue for conflicted requirements and unresolved paid-extra decisions under no-extras profiles. | Cancellation insurance showed the verifier could claim satisfied while current page evidence still said pending. | Add replay regression for classifier says pending + verifier claims satisfied. |
...
| P1.4 | Improve overlay detection. | `[x]` | Surface stack, `currentSurface`, background task pausing, backend current-surface target binding, foreground confidence, foreground visual fingerprint, progress markers, accessibility evidence, and action-producing fallback removal are implemented. | Seat return-leg and seat-preference surfaces showed the planner must understand the visible foreground surface directly. | P1.2/P1.5 will add visual target IDs and non-DOM seat-map regions for stronger screenshot grounding. |
