...
Current phase: `P1.2 annotated canonical control IDs`
...
| P0.4 | Add transaction invariants. | `[x]` | Pre-payment invariants block payment/final actions, unsafe money actions, validation-error navigation, unresolved required groups/fields, stale active-surface/background navigation, and observation-hash drift. Continue blocking now consumes canonical decision-group requirements instead of broad section/label state. Non-required paid alternatives no longer block Continue just because the profile says no extras. | Recent flexible-ticket/cancellation tests showed recovery could replace a correct Continue with a stale/broad decline search, and unselected paid alternatives could be treated as missing. | Keep payment authorization separate in P0.6 before any real payment/final purchase action. |
...
| P0.7 | Prevent stale-target execution after page changes. | `[x]` | Target snapshots cover active-surface buttons/options, page buttons/fields, and section buttons/choices/fields, with target kind, semantic, risk, surface, section, required/value metadata. Extension validates live target identity/surface/label/box/coverage and rejects observation-hash drift before execution. Material observation text/hash paths exclude injected agent UI/cursor overlays. | Repeated labels, active modals, explanatory target labels, and cursor overlays exposed target binding/hash issues. | P1.2 will add visual target IDs on screenshots for stronger model grounding, but stale-target execution is now guarded. |
| P0.8 | Evidence-backed requirement reconciliation. | `[x]` | Blind `satisfiedRequirementIds` merging is removed. Requirement evidence is checked against canonical decision groups so evidence from one group cannot satisfy another group. Authority order is deterministic current group/control state -> same-observation verifier evidence -> classifier/verifier interpretation -> historical memory. Regression tests now cover cross-group evidence rejection, deterministic group state outranking stale verifier claims, and navigation blocking for missing required groups. | Baggage requirement was previously verified using a flexible-ticket control because evidence was scoped to control text, not decision group. | Keep adding captured failures to the P2 replay corpus, but reconciliation is now evidence-backed. |
| P0.9 | Canonical logical control graph. | `[x]` | DOM/ARIA logical controls now flow from extension observation through server compaction, backend target binding, extension target resolution, validation, and verification. Logical `controlId`s no longer include transient DOM element IDs, and custom dropdown/combobox controls expose selected values as control state. Regression tests prove repeated labels bind by canonical `controlId` and target snapshots preserve decision-group identity. | Radio/label/custom-card failures showed raw DOM node identity was too brittle. | P1.2 will annotate screenshots with these same `controlId`s. P1.5 extends the graph to canvas/SVG/seat-map regions. |
| P0.10 | Canonical decision/requirement groups. | `[x]` | Extension builds `page.decisionGroups`; every logical choice/dropdown control carries `decisionGroupId`; server compaction preserves groups; backend target snapshots/actions carry group identity; requirement reconciliation and extension verification use group-scoped evidence; target validation rejects cross-group target execution. One logical `decisionGroupId` now survives collapsed control -> open popup -> reopened popup, and starting mid-checkout adopts the current selected dropdown state. Backend synthesizes canonical group-backed requirements and filters duplicate classifier choice/paid-extra requirements for the same group. | Root issue: controls were canonical, but decision identity still used transient DOM IDs for custom dropdown/listbox representations. One flexible-ticket decision could look like several unrelated groups. | Keep replaying captured airline failures; the next visual grounding step is P1.2 annotated screenshots with the same canonical IDs. |
...
| P1.4 | Improve overlay detection. | `[~]` | Surface stack, `currentSurface`, background task pausing, backend current-surface target binding, foreground confidence, foreground visual fingerprint, progress markers, accessibility evidence, action-producing fallback removal, and parent decision-group inheritance for opened dropdown/listbox surfaces are implemented. | Reopened because a reopened dropdown/choice surface could be treated as blocking even when its selected value was already correct. | Continue hardening overlay detection for arbitrary airline modals, especially canvas/SVG seat maps and nested iframes. |
...
| 2026-07-13 | Group-first invariants | Finished the P0.10/P0.4 implementation pass. Backend now turns decision groups into the canonical choice requirements, filters duplicate classifier requirements for the same group, and policy no longer treats every non-required paid alternative as a no-extras blocker. | P0.4 and P0.10 marked done. Remaining proof work lives in P0.8/P0.9 replay tests and P1.4 overlay/foreground behavior. |
| 2026-07-13 | P0.8/P0.9 replay regression tests | Added Node regression tests for decision-group reconciliation, cross-group evidence rejection, missing-group navigation blocking, repeated-label canonical `controlId` binding, and preservation of `decisionGroupId` in target snapshots. | P0.8 and P0.9 marked done for DOM/ARIA + backend contract. P1.2/P1.5 remain open for screenshot annotations and canvas/SVG regions. |
| 2026-07-13 | P0.10 dropdown continuity | Added stable decision-group identity across collapsed custom dropdowns, opened listboxes, and reopened listboxes. Closed dropdown selected values now become current group state, open surfaces inherit the parent decision group, logical control IDs no longer include transient DOM IDs, and material hashes exclude injected agent UI. | P0.10 remains done with stronger acceptance criteria; supporting P0.9/P0.8/P0.7/P1.4 work landed in the same pass. |
