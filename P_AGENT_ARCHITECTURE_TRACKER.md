...
| P0.9 | Canonical logical control graph. | `[x]` | DOM/ARIA logical controls now flow from extension observation through server compaction, backend target binding, extension target resolution, validation, and verification. Regression tests now prove repeated labels bind by canonical `controlId` and target snapshots preserve decision-group identity. | Radio/label/custom-card failures showed raw DOM node identity was too brittle. | P1.2 will annotate screenshots with these same `controlId`s. P1.5 extends the graph to canvas/SVG/seat-map regions. |
| P0.10 | Canonical decision/requirement groups. | `[x]` | Extension builds `page.decisionGroups`; every logical choice control carries `decisionGroupId`; server compaction preserves groups; backend target snapshots/actions carry group identity; requirement reconciliation and extension verification use group-scoped evidence; target validation rejects cross-group target execution. Backend now synthesizes canonical group-backed requirements and filters out duplicate classifier choice/paid-extra requirements for the same group. | Root issue: controls were canonical, but baggage/flex/cancellation decisions were not. Unselected paid alternatives inside a satisfied group were treated as missing. | Replay-test the captured baggage, flexible-ticket, cancellation, and seat-preference failures under P0.8/P0.9 coverage. |
