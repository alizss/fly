...
function compactTaskState(taskState = null) {
  ...
  return {
    ...
    attestationReceipts: eventSummary(taskState.attestationReceipts || []),
    ...
    sceneId: String(taskState.sceneId || ""),
...
function compactExecutionEpisode(state = {}) {
  ...
  return {
    ...
    navigation: current.navigation === true,
    navigationEpisodeId: String(current.navigationEpisodeId || ""),
    navigationStatus: String(current.navigationStatus || ""),
    sourceDocument: eventSummary(current.sourceDocument || null),
    destinationDocument: eventSummary(current.destinationDocument || null),
    destinationObservation: eventSummary(current.destinationObservation || null),
    expectedPostcondition: eventSummary(current.expectedPostcondition || null),
    destinationReadiness: eventSummary(current.destinationReadiness || null),
    deadlineAt: Math.max(0, Number(current.deadlineAt || 0)),
...
function compactSessionState(state = {}) {
  ...
  return {
    ...
    // The checkout mandate is durable transaction authority, not ephemeral
    // page interpretation. Dropping it after the session handshake caused
    // every later standard carrier attestation to look exceptional.
    checkoutMandate: eventSummary(state.checkoutMandate || null),
